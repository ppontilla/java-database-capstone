...
function renderHeader() {
  ...
  // If on homepage/root, clear user session and show basic header only
  if (window.location.pathname.endsWith("/")) {
    localStorage.removeItem("userRole");
  ...
  // If logged in roles but missing token, session expired
  if ((role === "loggedPatient" || role === "admin" || role === "doctor") && !token) {
    localStorage.removeItem("userRole");
    alert("Session expired or invalid login. Please log in again.");
...
// Attach event listeners to buttons that exist after rendering
...
// Logout for admin and doctor roles - clears everything and redirects to homepage
function logout() {
  localStorage.removeItem("userRole");
  localStorage.removeItem("token");
  window.location.href = "/";
...
// Logout for patient role - clears token, sets role to 'patient' (not logged in), redirects to patient dashboard
